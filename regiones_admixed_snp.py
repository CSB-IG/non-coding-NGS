...
admixed_sets = {}
for s in admixed:
    admixed_sets[s] = set([v.strip() for v in open( "maps/%s_snp.map" % ethnicity_code[s]).readlines()])
...
venn3(siete_regiones(admixed_union, centre_union, peninsula_union),
      set_labels=('Admixed', 'Centre', 'South'),ax=axes[0])
venn3(siete_regiones(north_union, admixed_union, peninsula_union),
      set_labels=('North', 'Admixed', 'South'),ax=axes[1])
venn3(siete_regiones(north_union, centre_union, admixed_union),
      set_labels=('North', 'Centre', 'Admixed'),ax=axes[2])

figure.set_size_inches(9,3)
figure.set_dpi(200)

plt.savefig('regiones_admixed_snp.png')
